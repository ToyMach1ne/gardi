...
(function (a, b, c, d) {
    ...
    function V(e) {
        ...
        function cI(a, b, d) {
            ...
            if (g) {
                f().vertical ? (H = V, J = L) : (H = L, J = V), bR.add(cl).animate({width: L, height: V}, b), f().thumbs && f().vertical && (f().thumbsOnRight ? cp.animate({left: L}, b) : bR.animate({left: !bq || !f().hideNavIfFullscreen ? cq : 0}, b));
                var i;
                f().touchStyle ? (bz = (H + f().margin) * p - f().margin, bA = J, i = {}, i[bP] = bz, i[bQ] = bA, bS.animate(i, b).data(i).data({minPos: -(bz - H), maxPos: 0})) : bS.animate({width: L, height: V}, b), f().thumbs && (f().vertical ? cp.animate({height: V}, b) : cp.animate({width: L}, b), f().thumbsPreview && e && cS(b, d), cp.css({visibility: "visible"})), r && !f().vertical && (f().arrows && bY.animate({top: V / 2}, b), i = {}, i[bM] = J / 2, bT.animate(i, b));
        ...
        function cO(a, b, c, d) {
            ...
            cM(a), d ? (e = d, u && f().cssTransitions && a.data({backAnimate: setInterval(function () {
        ...
        function cQ(a, b, c) {
            ...
            if (cs) {
                ...
                if (!g && cy)cx.hide(), cy = !1; else {
                    ...
                    a *= .9, u && f().cssTransitions ? (cx.css(N(a)), cx.css(K(r, f().vertical)).css(bP, q)) : f().vertical ? cx.stop().animate({top: r, height: q}, a, x) : cx.stop().animate({left: r, width: q}, a, x)
                }
            }
        }
        ...
        var j = new g({length: 8, radius: 6, width: 2, color: f().spinnerColor, rotate: 15}), k, l = f().data && (typeof f().data == "object" || typeof f().data == "string");
        ...
        k.each(function (a) {
            if (!l) {
                ...
                if (!b.data("html")) {
                    var d = b.children();
                    t[a] = {imgHref: b.attr("href"), imgSrc: b.attr("src"), thumbSrc: d.attr("src"), imgRel: b.attr("rel") ? b.attr("rel") : d.attr("rel")}, this.caption = b.attr("alt") || d.attr("alt")
                } else t[a] = {imgHref: b.attr("data-img") || " ", thumbSrc: b.attr("data-thumb"), imgRel: b.attr("data-full")}, this.caption = this.caption || b.attr("data-caption"), f().html || (f().html = {}), f().html[a] = b, r && (f()._html || (f()._html = {}), f()._html[a] = b.html())
            } else t[a] = {imgHref: this.img ? this.img : this.href ? this.href : this.length ? String(this) : null, thumbSrc: this.thumb, imgRel: this.full}
        ...
        f().arrows || (f().loop = !0);
        var G = [], H, J, L, V, W, X, Y = O(f().minWidth), Z = O(f().maxWidth), _ = O(f().minHeight), ba = O(f().maxHeight), bb = 1, bc = !0, bd = !0, be, bf = {no: /^[0-9.]+$/, px: /^[0-9.]+px$/, "%": /^[0-9.]+%$/, i: /^ё\d+$/}, bg, bh, bi, bj, bk, bl, bm, bn, bo, bp, bq, br, bs, bt, bu, bv, bw, bx, by;
        ...
        o && c(I("svg", {viewBox: "0 0 32 32"})).append(c(I("g", {fill: "none", stroke: f().spinnerColor, "stroke-width": 2})).append(I("circle", {cx: 16, cy: 16, r: 13})).append(I("line", {x1: 7, y1: 7, x2: 25, y2: 25}))).appendTo(bT), f().noise && c('<div class="fotorama__noise"></div>').prependTo(bR);
        var bU;
    ...
    !function (a, b, c) {
        ...
        }(), p = {lines: 12, length: 7, width: 5, radius: 10, rotate: 0, corners: 1, color: "#000", speed: 1, trail: 100, opacity: .25, fps: 20, zIndex: 2e9, className: "spinner", top: "auto", left: "auto"};
        g = function (a) {
            if (!this.spin)return new g(a);
            this.opts = n(a || {}, g.defaults, p)
        }, g.defaults = {}, n(g.prototype, {spin: function (a) {
            this.stop();
            var b = this, c = b.opts, d = b.el = m(h(0, {className: c.className}), {position: "relative", width: 0, zIndex: c.zIndex}), e = c.radius + c.length + c.width, g, i;
            a && (a.insertBefore(d, a.firstChild || null), i = o(a), g = o(d), m(d, {left: (c.left == "auto" ? i.x - g.x + (a.offsetWidth >> 1) : parseInt(c.left, 10) + e) + "px", top: (c.top == "auto" ? i.y - g.y + (a.offsetHeight >> 1) : parseInt(c.top, 10) + e) + "px"})), d.setAttribute("aria-role", "progressbar"), b.lines(d, b.opts);
            if (!f) {
            ...
            function e(a, d) {
                return m(h(), {position: "absolute", width: b.length + b.width + "px", height: b.width + "px", background: a, boxShadow: d, transformOrigin: "left", transform: "rotate(" + ~~(360 / b.lines * c + b.rotate) + "deg) translate(" + b.radius + "px" + ",0)", borderRadius: (b.corners * b.width >> 1) + "px"})
            }

            var c = 0, d;
            for (; c < b.lines; c++)d = m(h(), {position: "absolute", top: 1 + ~(b.width / 2) + "px", transform: b.hwaccel ? "translate3d(0,0,0)" : "", opacity: b.opacity, animation: f && k(b.opacity, b.trail, c, b.lines) + " " + 1 / b.speed + "s linear infinite"}), b.shadow && i(d, m(e("#000", "0 0 4px #000"), {top: "2px"})), i(a, i(d, e(b.color, "0 0 1px rgba(0,0,0,.1)")));
            return a
            ...
            !l(b, "transform") && b.adj ? (j.addRule(".spin-vml", "behavior:url(#default#VML)"), g.prototype.lines = function (b, c) {
                ...
                function k(b, e, g) {
                    i(h, i(m(f(), {rotation: 360 / c.lines * b + "deg", left: ~~e}), i(m(a("roundrect", {arcsize: c.corners}), {width: d, height: c.width, left: c.radius, top: -c.width >> 1, filter: g}), a("fill", {color: c.color, opacity: c.opacity}), a("stroke", {opacity: 0}))))
                }

                var d = c.length + c.width, e = 2 * d, g = -(c.width + c.length) * 2 + "px", h = m(f(), {position: "absolute", top: g, left: g}), j;
